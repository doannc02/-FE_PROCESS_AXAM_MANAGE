...
import { displayTitleHis, paymentStatusEnum } from '@/enum'
...
const InvoiceList = () => {
  const [values, handles] = useInvoiceList()
  ...
  return (
    <PageContainer
      title={
        <CoreBreadcrumbs
          ...
          breadcrumbs={[
            {
              title: displayTitleHis[typeInvoiceOrRefund({ router })],
            },
          ]}
        />
      }
    >
      ...
      <div className='py-4 flex justify-end gap-4 items-center'>
        ...
        <CoreButton onClick={() => router.push(`${typePathRedirect}/addNew`)}>
          {t('common:btn.add')}
        </CoreButton>
      </div>
